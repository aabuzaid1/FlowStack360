...
export class AuthService {
  ...
  private async loginOrRegisterProvider(
    ...
    const userByEmail = await this._userService.getUserByEmail(providerUser.email);
    if (userByEmail) {
      return userByEmail;
    }
  ...
  async changePassword(userId: string, currentPassword: string, newPassword: string) {
    ...
    // If user has a password (local account), verify current password
    if (user.password && currentPassword) {
      if (!AuthChecker.comparePassword(currentPassword, user.password)) {
        throw new Error('Current password is incorrect');
      }
    } else if (user.password && !currentPassword) {
      throw new Error('Current password is required');
    }
    return this._userService.updatePassword(userId, newPassword);
  }

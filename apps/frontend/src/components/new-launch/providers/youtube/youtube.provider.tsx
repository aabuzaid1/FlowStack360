...
export default withProvider({
  ...
  checkValidity: async (items) => {
    // items is array of posts, each post is array of media
    const firstPost = items?.[0];
    const firstMedia = Array.isArray(firstPost) ? firstPost[0] : firstPost;

    if (!firstMedia) {
      return 'You need one media (video mp4) to publish to YouTube';
    }

    const path: string = firstMedia?.path ?? '';
    if (path.indexOf('mp4') === -1) {
      return 'Item must be a video (mp4) — please add a video file to the post content area, not the thumbnail';
    }
    return true;
  },

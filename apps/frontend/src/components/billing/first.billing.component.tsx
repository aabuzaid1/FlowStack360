...
import { mergeAdminPricing } from '@gitroom/frontend/components/billing/pricing-overrides';
...
export const FirstBillingComponent = () => {
  ...
  const showYouTube = () => {
    modals.openModal({
      title: 'Grow Fast With FlowStack360 (Play the video)',
      children: (
        <iframe
          className="h-full aspect-video min-w-[800px]"
          src="https://www.youtube.com/embed/BdsCVvEYgHU?si=vvhaZJ8I5oXXvVJS?autoplay=1"
          title="FlowStack360 Tutorial"
          allow="autoplay"
          allowFullScreen
        />
      ),
  ...
  const { data: planPricing } = useSWR('/billing/pricing', loadPricing);

  const billingPricing = useMemo(
    () => mergeAdminPricing(pricing, planPricing),
    [planPricing]
  );

  const price = useMemo(
    () =>
      Object.entries(billingPricing).filter(
        ([key, value]) => key !== 'FREE'
      ),
    [billingPricing]
  );
      ...
      <>
        <div className="text-[46px] font-[600] leading-[110%] tablet:text-[36px] mobile:!text-[30px] whitespace-pre-line text-balance">
          ...
          {t(
            ...
            'FlowStack360 To Grow Their Social Presence'
          )}
        </div>
        ...
        <div className="flex" onClick={showYouTube}>
          <div className="tablet:mb-[32px] cursor-pointer mt-[32px] flex gap-[10px] items-center underline hover:font-[700]">
            ...
            <div>See the power of FlowStack360 (click here)</div>
          </div>
        </div>
        ...
        <div className="flex flex-col ps-[40px] tablet:!ps-[0] border-l border-newColColor py-[40px] mobile:!pt-[24px] tablet:border-none tablet:pb-0">
          <div className="top-[20px] sticky">
            ...
            <div className="flex flex-col mt-[54px] gap-[24px] tablet:mt-[40px]">
              ...
              <BillingFeatures tier={tier} pricingMap={billingPricing} />
            </div>
...
export const BillingFeatures: FC<{
  tier: string;
  pricingMap: typeof pricing;
}> = ({ tier, pricingMap }) => {
  const t = useT();
  const features = useMemo(() => {
    const currentPricing = pricingMap[tier];

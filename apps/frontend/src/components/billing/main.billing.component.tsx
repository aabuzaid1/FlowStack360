...
import {
  AdminPricingRow,
  mergeAdminPricing,
} from '@gitroom/frontend/components/billing/pricing-overrides';
...
export const Features: FC<{
  ...
  pricingMap: PricingInterface;
}> = (props) => {
  const { pack, pricingMap } = props;
  const features = useMemo(() => {
    const currentPricing = pricingMap[pack];
...
export const MainBillingComponent: FC<{
  sub?: Subscription;
  planPricing?: AdminPricingRow[];
}> = (props) => {
  const { sub, planPricing } = props;
  ...
  const moveToCheckout = useCallback(
    (billing: 'STANDARD' | 'PRO' | 'FREE', reactivate = false) =>
      async () => {
        ...
        const messages = [];
        if (
          !billingPricing[billing].team_members &&
          billingPricing[subscription?.subscriptionTier!]?.team_members
        ) {
          messages.push(
    ...
    [billingPricing, monthlyOrYearly, subscription, user, utm]
  );
  if (user?.isLifetime) {
    router.replace('/');
    return null;

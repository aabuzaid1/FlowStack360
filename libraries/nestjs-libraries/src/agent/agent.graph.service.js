...
const contentZod = (isPicture, format) => {
    const content = z.object(Object.assign({ content: z.string().describe('Content for the new post'), website: z
            .string()
            .nullable()
            .optional()
            .describe("Website for the new post if exists, If one of the post present a brand, website link must be to the root domain of the brand or don't include it, website url should contain the brand name") }, (isPicture
        ? {
            prompt: z
                .string()
...
let AgentGraphService = AgentGraphService_1 = class AgentGraphService {
    ...
    startCall(state) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const response = yield ChatPromptTemplate.fromTemplate(`
    ...
    findCategories(state) {
        return __awaiter(this, void 0, void 0, function* () {
            const allCategories = yield this._postsService.findAllExistingCategories();
            const structuredOutput = model.withStructuredOutput(category);
            const { category: outputCategory } = yield ChatPromptTemplate.fromTemplate(`
    ...
    findTopic(state) {
        return __awaiter(this, void 0, void 0, function* () {
            const allTopics = yield this._postsService.findAllExistingTopicsOfCategory(state === null || state === void 0 ? void 0 : state.category);
            if (allTopics.length === 0) {
                return { topic: null };
            }
            const structuredOutput = model.withStructuredOutput(topic);
            const { topic: outputTopic } = yield ChatPromptTemplate.fromTemplate(`
        You are an assistant that gets a text that will be later summarized into a social media post
    ...
    findPopularPosts(state) {
        return __awaiter(this, void 0, void 0, function* () {
            const popularPosts = yield this._postsService.findPopularPosts(state.category, state.topic);
            return { popularPosts };
        });
    }
    generateHook(state) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const { hook: outputHook } = yield ChatPromptTemplate.fromTemplate(`
    ...
    generateContent(state) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const { content: outputContent } = yield ChatPromptTemplate.fromTemplate(`
    ...
    generatePictures(state) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!state.isPicture) {
                return {};
            }
            const newContent = yield Promise.all((state.content || []).map((p) => __awaiter(this, void 0, void 0, function* () {
                const image = yield dalle.invoke(p.prompt);
                return Object.assign(Object.assign({}, p), { image });
            })));
            return {
                content: newContent,
            };
    ...
    uploadPictures(state) {
        return __awaiter(this, void 0, void 0, function* () {
            const all = yield Promise.all((state.content || []).map((p) => __awaiter(this, void 0, void 0, function* () {
                if (p.image) {
                    const upload = yield this.storage.uploadSimple(p.image);
                    const name = upload.split('/').pop();
                    const uploadWithId = yield this._mediaService.saveFile(state.orgId, name, upload);
                    return Object.assign(Object.assign({}, p), { image: uploadWithId });
                }
                return p;
            })));
            return { content: all };
    ...
    postDateTime(state) {
        return __awaiter(this, void 0, void 0, function* () {
            return { date: yield this._postsService.findFreeDateTime(state.orgId) };
        });
    }
    start(orgId, body) {
...
AgentGraphService.state = () => new StateGraph({
    channels: {
        messages: {
...
AgentGraphService = AgentGraphService_1 = __decorate([
    Injectable(),
    __metadata("design:paramtypes", [PostsService,
        MediaService])
], AgentGraphService);
export { AgentGraphService };

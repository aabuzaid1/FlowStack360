  ...
  async checkAnalytics(
    ...
    if (integrationProvider.analytics) {
      try {
        ...
        const logger = new (require('@nestjs/common').Logger)('IntegrationService');
        logger.error(`Analytics fetch failed for ${getIntegration.providerIdentifier} (${integration}): ${e?.message || e}`);
        if (e?.response?.data) {
          logger.error(`API Response: ${JSON.stringify(e.response.data)}`);
        }
      }
    }

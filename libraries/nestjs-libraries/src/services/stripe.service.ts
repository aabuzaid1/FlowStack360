...
export class StripeService {
  ...
  private async getEffectivePricing(tier: string) {
    const row = await this._adminService.getPricingForTier(tier);

    return {
      ...pricing[tier],
      month_price: row.monthPrice,
      year_price: row.yearPrice,
      channel: row.totalChannels,
    };
  }
  ...
  async createSubscription(event: Stripe.CustomerSubscriptionCreatedEvent) {
    ...
    const plan = await this.getEffectivePricing(billing);
  ...
  async prorate(organizationId: string, body: BillingSubscribeDto) {
    ...
    const priceData = await this.getEffectivePricing(body.billing);
  ...
  async lifetimeDeal(organizationId: string, code: string) {
    ...
    try {
      ...
      const findPricing = await this.getEffectivePricing(nextPackage);
